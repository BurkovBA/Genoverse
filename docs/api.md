...
Genoverse does not use underscored field names, such as `_private` or `_privateFunction()`, to denote private fields. Generally, all variables on Genoverse or Genoverse.Track objects can be considered private in a sense that it is not safe to modify them directly, saying e.g `genoverse.start = 1`. Instead, call the following API functions.
